...
class Field_model(Conform_model):
    ...
    char_choice = CharField(default='Char one text', choices={'char1':'Char one text', 'char2':'Char two text', 'char3': 'Char three text'} , verbose_name='Char with choices', help_text="A field to store a char in database")
    ...
    foreignkey = ForeignKeyField(model=Fk_model, null=True, verbose_name='ForeignKey', help_text='A field to store a foreignkey in database', filter_fields=['options'] ,choice_query={'options__eq': True})
    int = IntegerField(default=1000 ,verbose_name='Integer', help_text='A field to store a integer in database')
    int_choice = IntegerField(default=1, choices={'int1':1, 'int2':2, 'int3': 3} , verbose_name='Integer with choices', help_text="A field to store a integer in database")
    manytomany = ManyToManyField(model=Mm_model, backref='test_manytomany', verbose_name='ManyToMany', help_text='A field to store a manytomany in database',filter_fields=['options'] ,choice_query={'options__eq': True})
    mm_backrefs = BackrefMetaField(help_text='A field to access manytomany backrefs in database', verbose_name='ManyToMany backrefs', choice_query={'options__eq': True})
