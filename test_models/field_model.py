...
class Field_model(Conform_model):
    ...
    foreignkey = ForeignKeyField(model=Fk_model, null=True, verbose_name='ForeignKey', help_text='A field to store a foreignkey in database', filter_fields=['options'] ,options_query={'options__eq': True})
    int = IntegerField(default=1000 ,verbose_name='Integer', help_text='A field to store a integer in database')
    manytomany = ManyToManyField(model=Mm_model, backref='test_manytomany', verbose_name='ManyToMany', help_text='A field to store a manytomany in database',filter_fields=['options'] ,options_query={'options__eq': True})
